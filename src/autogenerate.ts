...
function writeServers(
  ...
  services.forEach((service) => {
    result += `export function ${service.name}Handler(service: ${service.name}) { return {
    path: '${packageName}',
    methods: {\n`;
    service.methods.forEach((method) => {
      result += `${method.name}: createMethodHandler({ handler: service.${method.name}, encode: ${method.input}, decode: ${method.output} }),`;
    });
  });
  result += "}\n";
  result += "}\n";
