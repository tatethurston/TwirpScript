...
import { Empty, EmptyJSON } from "../google/protobuf/empty.pb";
...
export interface Haberdasher<Context = unknown> {
  GetAll: (empty: Empty, context: Context) => Promise<HatList> | HatList;
  /**
   * MakeHat produces a hat of mysterious, randomly-selected color!
   */
...
export const HatList = {
  ...
  decode: function (bytes: ByteSource): HatList {
    return HatList._readMessage(HatList.initialize(), new BinaryReader(bytes));
  },

  /**
   * Initializes HatList with all fields set to their default value.
   */
  initialize: function (): HatList {
    return {
      hats: [],
    };
  },
   ...
   * @private
   */
  _readMessage: function (msg: HatList, reader: BinaryReader): HatList {
    while (reader.nextField()) {
      const field = reader.getFieldNumber();
      switch (field) {
        case 1: {
...
export const HatListJSON = {
  ...
  _readMessage: function (msg: HatList, json: any): HatList {
    const _hats = json["hats"];
    if (_hats) {
      for (const item of _hats) {
        const m = Hat.initialize();
